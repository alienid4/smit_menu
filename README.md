...
| `run_impact_cmd` | `[IMPACT]` 紅 | **雙重確認**：`CONFIRM` + 打主機名 (10 秒 timeout) | systemctl restart, kill -9 |
...
**失敗類型都進 audit log**：
- `CANCEL 1st-confirm fail: <打錯的值>`
- `CANCEL 2nd-confirm timeout`
- `CANCEL 2nd-confirm mismatch: '<打錯>' vs '<正確>'`
...
|---|---|---|---|---|
| RHEL / Rocky / Alma / CentOS Stream | ≥ 8 | `dnf` | `faillock` | `firewall-cmd` |
| RHEL / CentOS 7 | 7 | `yum` | `pam_tally2` | `firewall-cmd` |
| Ubuntu / Debian / LinuxMint | 所有 | `apt` | `pam_tally2` | `ufw` |
...
DISTRO_PRETTY        # "Rocky Linux 9.7 (Blue Onyx)"
```
